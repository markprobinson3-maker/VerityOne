...
# 13. `vo` CLI shim. There is no packaged `vo` binary — the CLI lives in the repo
# at agent-lab/scripts/vo-cli.ts. Install a launcher at ~/.local/bin/vo that execs
# it, so the `vo ...` commands the website + docs show work verbatim after a source
# install (instead of "command not found").
...
============================================================
  ...
  vo CLI:        $BIN_DIR/vo${VO_ON_PATH:+  (on PATH ✓)}${VO_ON_PATH:+}
EOF
if [ -z "$VO_ON_PATH" ]; then
  cat <<EOF
                 ^ $BIN_DIR is NOT on your PATH yet — add it so \`vo\` resolves:
