...
__export(exports_src, {
  withEmbedRetry: () => withEmbedRetry,
  toVectorStr: () => toVectorStr,
  embeddingTextForNode: () => embeddingTextForNode,
  embeddingMetadataForVectorWrite: () => embeddingMetadataForVectorWrite,
  embedTextForInsertFromCache: () => embedTextForInsertFromCache,
  embedTextForInsert: () => embedTextForInsert,
  embedQuery: () => embedQuery,
  embedCacheStats: () => embedCacheStats,
  embedBatchWithCacheMetadata: () => embedBatchWithCacheMetadata,
  embedBatch: () => embedBatch,
  assertEmbedDim: () => assertEmbedDim,
  EmbedDimensionMismatchError: () => EmbedDimensionMismatchError,
  EmbedApiError: () => EmbedApiError,
...
`), enumerable: options.debug },
  ...
  async function SASLContinue(x) {
    ...
    const saltedPassword = await import_crypto.default.pbkdf2Sync(await Pass(), Buffer.from(res.s, "base64"), parseInt(res.i), 32, "sha256");
...
// node_modules/.bun/@verity-one+db-pool@file+packages+db-pool/node_modules/@verity-one/db-pool/src/index.js
function getDatabaseUrl() {
  const configured = process.env.DATABASE_URL?.trim();
  return configured || "postgresql://localhost:5432/verity";
}
function createPooledSql(extra = {}) {
  return src_default(getDatabaseUrl(), {
    max: 2,
    idle_timeout: 20,
    connect_timeout: 5,
    prepare: false,
...
function assertEmbedDim(values2) {
  if (!values2) {
    throw new EmbedDimensionMismatchError(0, EMBED_DIMS);
  }
  if (values2.length !== EMBED_DIMS) {
    throw new EmbedDimensionMismatchError(values2.length, EMBED_DIMS);
  }
}
var _cacheSql = null;
function cacheSql() {
...
async function fetchEmbedQuery(text) {
  ...
  const values2 = data.embedding.values;
  assertEmbedDim(values2);
  return values2;
}
